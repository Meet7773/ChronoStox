...
# ===============================================================
# EXECUTION & PORTFOLIO PARAMS
# ===============================================================
SLIPPAGE_BPS = 10  # 0.1% slippage per trade
BROKERAGE_BPS = 3  # 0.03% broker fee
STT_BPS = 10  # 0.1% STT (applied on sell only for equity delivery)
EXCHANGE_FEES_BPS = 0.35  # NSE txn charges
...
# Cleaned up static list: Only use a tiny 5-stock fallback if NSE site is down
DEFAULT_TICKERS = ["RELIANCE.NS", "HDFCBANK.NS", "ICICIBANK.NS", "INFY.NS", "TCS.NS"]


def get_dynamic_tickers():
    """Scrapes the live Nifty 200 constituents directly from NSE archives."""
    try:
        print("[+] Fetching Top 200 dynamic index constituents from NSE...")
        ...
        print("⚠️ Falling back to DEFAULT_TICKERS.")
...
def load_sentiment_once(data_dir):
    ...
    if "Date" in df.columns:
        ...
        print("FATAL: Sentiment missing Date");
        sys.exit(1)
...
def _run_portfolio_sim(results_df, signal_mode="combined", label=""):
    ...
    # Track the macro regime to wipe the blacklist when the market flips
    ...
    for sim_date, group in grouped:
        ...
        # --- MACRO AMNESIA (Regime Reset) ---
        # Get the dominant regime for this week
        current_macro = group[regime_col].mode()[0] if regime_col in df.columns else "NEUTRAL"
        if pd.isna(current_macro): current_macro = "NEUTRAL"

        if current_macro != prev_macro_regime:
            ticker_track.clear()  # Wipe the blacklist clean on regime change!
        ...
        for _, row in group.iterrows():
            ...
            if signal_mode == "regime":
                ...
                if regime == "BULL":
                    ...
                    if meta_signal == "HOLD" or hybrid_signal == "HOLD": continue
                    if meta_signal != hybrid_signal: continue
                ...
                if regime == "BULL":
                    ...
                    if meta_signal != hybrid_signal:
                        ...
                        continue
                    if meta_signal == "SELL" and confidence < 0.05: continue
                    trade_signal = meta_signal

                if trade_signal == "HOLD": continue

                # 2. Strict Filters (Bypassed if the Macro Regime forced an Override)
                if not is_override:
                    if confidence < 0.08:
                    ...
                    if meta_5d_col in df.columns:
                        ...
                        if pd.isna(meta_5d): meta_5d = "HOLD"
                ...
                # 3. Dynamic Quality Filter (Now wiped when regimes change)
                ...
                # Boost confidence artificially if macro overrides so position size doesn't cripple us
                ...
                if meta_signal == "HOLD": continue
                trade_signal = meta_signal
                raw_weight = 0.20

            elif signal_mode == "hybrid":
                if hybrid_signal == "HOLD": continue
                trade_signal = hybrid_signal
                raw_weight = 0.20

            if trade_signal != "HOLD":
                proxy_vol = max(abs(hybrid_ret), 0.01)
                risk_adj_weight = raw_weight / proxy_vol
                risk_adj_weight = min(risk_adj_weight, 5.0)

                if trade_signal == "SELL":
                    target_weights[tkr] = -risk_adj_weight
                else:
                    target_weights[tkr] = risk_adj_weight

                ticker_alloc_info[tkr] = {
                    "signal": trade_signal,
                    "actual_fwd_ret": actual,
                }
        ...
        if total_abs_weight > 0.0:
            ...
            for tkr in target_weights:
                target_weights[tkr] *= scale_factor
        ...
        for tkr in all_tickers:
            old_w = current_positions.get(tkr, 0.0)
            new_w = target_weights.get(tkr, 0.0)
            weight_diff = new_w - old_w

            if weight_diff > 0:
                fee = weight_diff * TOTAL_BUY_FEE_PCT
            elif weight_diff < 0:
                fee = abs(weight_diff) * TOTAL_SELL_FEE_PCT
            else:
                fee = 0.0
            ...
            if new_w != 0.0:
                ...
                trade_sig = info["signal"]
                actual_ret = info["actual_fwd_ret"]
...
def print_report(metrics, portfolio_metrics, results_df, n_tickers, n_sim_dates, oos_start, oos_end):
    ...
    for h in HORIZONS:
        ...
        print(
            f"    BUY  : {m['hybrid_buy_count']} (win rate: {m['hybrid_buy_win_rate']}%, avg ret: {m['hybrid_buy_avg_ret']}%)")
        print(
            f"    SELL : {m['hybrid_sell_count']} (win rate: {m['hybrid_sell_win_rate']}%, avg ret: {m['hybrid_sell_avg_ret']}%)")
        print(f"    HOLD : {m['hybrid_hold_count']}")
        print(f"    --- Meta-Learner ---")
        if m['meta_accuracy'] != 'N/A':
            print(f"    Meta Accuracy     : {m['meta_accuracy']}%")
        print(
            f"    Meta BUY  : {m['meta_buy_count']} (win rate: {m['meta_buy_win_rate']}%, avg ret: {m['meta_buy_avg_ret']}%)")
        print(
            f"    Meta SELL : {m['meta_sell_count']} (win rate: {m['meta_sell_win_rate']}%, avg ret: {m['meta_sell_avg_ret']}%)")
    ...
    print("-" * 80)
    print(
        f"  {'Strategy':<18} | {'Return':>8} | {'MaxDD':>8} | {'Sharpe':>7} | {'Trades':>7} | {'Win%':>6} | {'Equity':>9}")
    print("  " + "-" * 80)

    # Order: combined_plus (recommended), then combined, regime_aware, meta, hybrid
    ...
    if "actual_ret_21d" in results_df.columns and "pred_ret_21d" in results_df.columns:
        ...
        print("-" * 80)
        print(
            f"  {'Ticker':<18} | {'N':>3} | {'DirAcc':>6} | {'MetaAcc':>7} | {'Pred':>6} | {'Actual':>7} | {'HMM Mode':>9}")
        print("  " + "-" * 72)
    ...
    for sim_date in sim_dates:
        for tkr, df_full_px in price_cache.items():
            ...
            if not past_slice.empty:
                ...
                past_res = [r for r in all_results if
                            r["ticker"] == tkr and r["sim_date"] == past_slice["Date"].iloc[-1]]
                if past_res:
                    p_res = past_res[0]
...
def main():
    ...
    parser.add_argument("--tickers", nargs="+", default=None,
                        help="Tickers to test (default: Dynamic NSE 200 via web scrape)")
    ...
    # Route default behavior to the new dynamic web scraper
    if args.tickers:
    ...
    print(f"\n{'=' * 60}")
    print(f"  ChronoStox v9.5 — OOS Backtester (Quant Overlay Edition)")
    print(f"{'=' * 60}")
